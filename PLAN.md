...
- [x] 종가 동기화 순서를 현재가 -> 자산 로그 -> 벤치마크 -> 검증으로 고정
- [x] 장시간 실행 중인 작업의 실패 판정과 작업별 재시도
- [x] 시장 동향 snapshot을 Supabase에 저장해 첫 화면 응답 개선
- [x] 공급자별 갱신 시각·오류·캐시 사용 여부 표시
- [x] 중복 Cron 실행에도 날짜별 로그가 하나만 유지되도록 검증
...
- [x] 조회 전용 계좌·국내주식·미국주식 잔고 연결 점검
- [x] 24시간 접근 토큰 Supabase 캐시와 계좌번호 마스킹
- [ ] 현재 Supabase 자산과 계좌별 차이 미리보기
- [ ] 승인한 NH 계좌만 자동 동기화
- [ ] 연금·IRP·일반 펀드 지원 여부 실계좌 검증
